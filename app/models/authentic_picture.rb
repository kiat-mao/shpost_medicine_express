class AuthenticPicture < ApplicationRecord
	enum status: {waiting: 'waiting', sended: 'sended', failed: 'failed'}
	STATUS_NAME = { waiting: '待发送', sended: '已发送', failed: '发送失败' }
    ...
    pkp_waybill_bases = PkpWaybillBase.where(sender_no: '1030001771000').where("biz_occur_date >= ? and biz_occur_date < ?", start_date, end_date)
	...
	def self.init_authentic_picture(pkp_waybill_base)
		authentic_picture = AuthenticPicture.find_or_initialize_by(express_no: pkp_waybill_base.express_no)
